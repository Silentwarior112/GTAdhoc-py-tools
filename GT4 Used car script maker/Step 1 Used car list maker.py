...
def select_files_and_process():
    ...
    response = messagebox.askquestion("Warning", "Make sure that you have accounted for special cases that appear in the used car dealership, and also somewhere else. Special cases include cars that only appear at specific weeks such as the black edition variants, or cars that appear in the used dealership, but have a prize-only special color variant, such as  mgf_vvc_97, etc. This also doesn't account for mx5_gms_05. If the model hasn't been restored from the MX-5 demo and integrated into the carlist somewhere, this needs to be manually added to the rule out list so that it doesn't get used.You can remove the special cases from the rule out list as needed, and make sure to take note of which cars you removed from it before continuing. That way, the next steps will generate the used car entries for them, then you can edit the outputted base list after step 5, and selectively remove the specific variations that you do not want to appear in the used dealership. Continue?", icon='warning')
